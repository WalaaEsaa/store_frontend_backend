...
export class Orders_store {
  async create(order: Orders): Promise<Orders> {
    ...
    const result = await conn.query(sql, [order.quantity, order.statuse, order.product_id, order.user_id])
  ...
  async updateOrders(id:number,quantity: number, statues: string, product_id: number, user_id: number): Promise<Orders> {
    try {
